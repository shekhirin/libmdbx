...
static int async_read_cursor_batches_result_func(void *context, const MDBX_val *pairs,
  ...
  if (!probe || !probe->values || !pairs || count == 0 || (count & 1))
    return MDBX_PROBLEM;
  for (size_t i = 0; i < count; i += 2) {
    if (probe->results >= ASYNC_READ_BATCH_COUNT)
      return MDBX_PROBLEM;
    ...
    uint64_t actual_key = UINT64_MAX;
    memcpy(&actual_key, key->iov_base, sizeof(actual_key));
    if (actual_key != probe->results ||
        memcmp(data->iov_base, probe->values[probe->results], ASYNC_READ_BATCH_VALUE_BYTES) != 0)
      return MDBX_PROBLEM;
    probe->results += 1;
  }
  probe->keys += 1;
...
static int async_read_cursor_scan_result_func(void *context, MDBX_val *key, MDBX_val *data,
  ...
  (void)arg;
  struct async_read_loop_probe *const probe = (struct async_read_loop_probe *)context;
  if (!probe || !probe->values || !key || !data || probe->results >= ASYNC_READ_BATCH_COUNT)
    return MDBX_PROBLEM;
  if (key->iov_len != sizeof(uint64_t) || data->iov_len != ASYNC_READ_BATCH_VALUE_BYTES)
    return MDBX_PROBLEM;
  uint64_t actual_key = UINT64_MAX;
  ...
  if (actual_key != probe->results ||
    ...
    return MDBX_PROBLEM;
  probe->results += 1;
  return probe->results == ASYNC_READ_BATCH_COUNT ? MDBX_RESULT_TRUE : MDBX_RESULT_FALSE;
}
...
static int exercise_async_read_path(const char *path, bool inject_fault, enum async_read_mode mode) {
  ...
  struct async_read_loop_probe abort_get_loop_probe = {0, 0, 0, 0, batch_values, MDBX_SUCCESS};
  struct async_read_loop_probe abort_get_ex_loop_probe = {0, 0, 0, 0, batch_values, MDBX_SUCCESS};
  struct async_read_loop_probe abort_lower_loop_probe = {0, 0, 0, 0, batch_values, MDBX_SUCCESS};
  struct async_read_loop_probe abort_cache_loop_probe = {0, 0, 0, 0, batch_values, MDBX_SUCCESS};
  struct async_read_loop_probe abort_cursor_loop_probe = {0, 0, 0, 0, batch_values, MDBX_SUCCESS};
  ...
  if (mode == async_read_sync_get) {
    ...
    read_name = "mdbx_cursor_get cold sync read";
    ...
    read_name = mode == async_read_cursor_get_range
                    ? "mdbx_async_cursor_get SET_RANGE cold single read"
                    : mode == async_read_cursor_get_upperbound
                          ? "mdbx_async_cursor_get SET_UPPERBOUND cold single read"
                    : mode == async_read_cursor_get_set
                          ? "mdbx_async_cursor_get SET cold single read"
                          : "mdbx_async_cursor_get cold single read";
    ...
    if (mode == async_read_sync_cursor_get_batch ||
      ...
      if (operation_result == MDBX_RESULT_TRUE)
        operation_result = MDBX_SUCCESS;
                ...
                mode == async_read_sync_cursor_scan_from ||
                mode == async_read_sync_cursor_scan_from_nodup) &&
               operation_result == MDBX_RESULT_TRUE) {
      operation_result = MDBX_SUCCESS;
    }
    rc = MDBX_SUCCESS;
  ...
  if (inject_fault) {
    if (mode == async_read_get) {
      ...
      REQUIRE(operation_result == MDBX_SUCCESS, "faulted async get loop operation returned wrong result");
      ...
      REQUIRE(operation_result == MDBX_SUCCESS, "faulted async get_ex loop operation returned wrong result");
      REQUIRE(loop_completed == ASYNC_READ_BATCH_COUNT,
    ...
    if (cache_single_read) {
      ...
      REQUIRE(read_key_value->iov_len == sizeof(batch_keys[2]),
              "cold sync cursor get upperbound returned wrong key size");
      uint64_t actual_key = UINT64_MAX;
      memcpy(&actual_key, read_key_value->iov_base, sizeof(actual_key));
      REQUIRE(actual_key == batch_keys[2],
              "cold sync cursor get upperbound returned wrong key");
      REQUIRE(data.iov_len == sizeof(batch_values[2]),
              "cold sync cursor get upperbound returned wrong value size");
      REQUIRE(memcmp(data.iov_base, batch_values[2], sizeof(batch_values[2])) == 0,
              "cold sync cursor get upperbound returned wrong value");
    } else if (mode == async_read_sync_cursor_get_nodup) {
      ...
      REQUIRE(read_key_value->iov_len == sizeof(batch_keys[2]),
              "cold async cursor get upperbound returned wrong key size");
      uint64_t actual_key = UINT64_MAX;
      memcpy(&actual_key, read_key_value->iov_base, sizeof(actual_key));
      REQUIRE(actual_key == batch_keys[2],
              "cold async cursor get upperbound returned wrong key");
      REQUIRE(data.iov_len == sizeof(batch_values[2]),
              "cold async cursor get upperbound returned wrong value size");
      REQUIRE(memcmp(data.iov_base, batch_values[2], sizeof(batch_values[2])) == 0,
              "cold async cursor get upperbound returned wrong value");
    } else if (mode == async_read_cursor_get_batch ||
               mode == async_read_cursor_get_batch_nodup ||
               mode == async_read_sync_cursor_get_batch ||
               mode == async_read_sync_cursor_get_batch_nodup) {
      ...
      for (unsigned i = cursor_batch_start_index; i < ASYNC_READ_BATCH_COUNT; ++i) {
        ...
        REQUIRE(memcmp(batch_data_value->iov_base, batch_values[i], sizeof(batch_values[i])) == 0,
               ...
               mode == async_read_cursor_get_loop_from ||
               mode == async_read_cursor_get_loop_from_nodup) {
      REQUIRE(cursor_stream_completed == ASYNC_READ_BATCH_COUNT,
    ...
    } else if (mode == async_read_cursor_scan || mode == async_read_cursor_scan_nodup ||
               mode == async_read_cursor_scan_from || mode == async_read_cursor_scan_from_nodup ||
               mode == async_read_sync_cursor_scan || mode == async_read_sync_cursor_scan_nodup ||
               mode == async_read_sync_cursor_scan_from ||
               mode == async_read_sync_cursor_scan_from_nodup) {
      REQUIRE(loop_read_probe.results == ASYNC_READ_BATCH_COUNT,
      ...
      REQUIRE(loop_completed == ASYNC_READ_BATCH_COUNT, "cold async get loop completed wrong count");
      REQUIRE(loop_read_probe.keys == ASYNC_READ_BATCH_COUNT, "cold async get loop prepared wrong key count");
      REQUIRE(loop_read_probe.results == ASYNC_READ_BATCH_COUNT, "cold async get loop saw wrong result count");
    } else if (cache_loop_read) {
      REQUIRE(loop_completed == ASYNC_READ_BATCH_COUNT, "cold async cache get loop completed wrong count");
      REQUIRE(loop_read_probe.keys == ASYNC_READ_BATCH_COUNT,
              "cold async cache get loop prepared wrong key count");
      REQUIRE(loop_read_probe.results == ASYNC_READ_BATCH_COUNT,
              "cold async cache get loop saw wrong result count");
    } else if (mode == async_read_get_ex_loop) {
      REQUIRE(loop_completed == ASYNC_READ_BATCH_COUNT, "cold async get_ex loop completed wrong count");
      REQUIRE(loop_read_probe.keys == ASYNC_READ_BATCH_COUNT,
