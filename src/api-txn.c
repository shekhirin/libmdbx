...
int mdbx_txn_clone(const MDBX_txn *source, MDBX_txn **dest) {
  ...
  const uint32_t snapshot_pages_used = atomic_load32(&source->ro.slot->snapshot_pages_used, mo_Relaxed);
  const uint64_t snapshot_pages_retired = atomic_load64(&source->ro.slot->snapshot_pages_retired, mo_Relaxed);
