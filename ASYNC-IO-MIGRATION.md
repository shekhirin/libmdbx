...
A later environment submit-descriptor cleanup removed
`dxb_env_check_fstat_submit_io_t`, `env_make_check_fstat_submit_io()`,
`env_check_fstat_submit_io_validate()`,
`dxb_env_sysv_lock_stat_submit_io_t`,
`env_make_sysv_lock_stat_submit_io()`,
`env_sysv_lock_stat_submit_io_validate()`,
`dxb_env_lck_readonly_probe_submit_io_t`,
`env_make_lck_readonly_probe_submit_io()`, and
`env_lck_readonly_probe_submit_io_validate()` from `mdbx.c`. DXB fstat checks,
SysV lock-file mode pickup, and read-only lock-file fallback probing now build
and validate the underlying storage stat/read-only submit descriptors directly
at their policy call sites. Verification passed `git diff --check`, a source
scan proving the removed environment submit helpers are absent from `mdbx.c`,
the GNUmake `mdbx_migration_smoke` build target, direct `mdbx_migration_smoke`
default and forced tiny-cache runs, the Ninja build (`cmake --build
@cmake-ninja-build`), the six focused `migration_smoke` CTest entries, the
full 15-test public migration CTest suite including API checks and tool
...
focused ASAN `migration_smoke` entries with `LSAN_OPTIONS=detect_leaks=0`.
The paired `mdbx_migration_bench_lazy` gate passed with forced/default ratios
of `1.109` batch, `1.143` crud, `0.814` iterate, `1.051` get, and `1.064`
delete.

A later dirty-write queue lifecycle cleanup removed
`dxb_iov_queue_prepare_submit_io_t`, `iov_make_queue_prepare_submit_io()`,
`iov_queue_prepare_submit_io_validate()`,
`dxb_iov_queue_reset_submit_io_t`, `iov_make_queue_reset_submit_io()`,
`iov_queue_reset_submit_io_validate()`, `dxb_iov_queue_walk_submit_io_t`,
`iov_make_queue_walk_submit_io()`, `iov_queue_walk_submit_io_validate()`,
`iov_dirty_write_queue_io_equal()`, and `iov_dirty_write_walk_io_equal()` from
`mdbx.c`. `iov_init()` and `iov_complete()` now build and validate the dirty
write-queue prepare, reset, and walk storage descriptors directly before
submitting them, while preserving the same data-channel and storage ownership
checks at the caller. Verification passed `git diff --check`, a source scan
proving the removed dirty-write queue lifecycle helpers are absent from
`mdbx.c`, the GNUmake `mdbx_migration_smoke` build target, direct
`mdbx_migration_smoke` default and forced tiny-cache runs, the Ninja build
(`cmake --build @cmake-ninja-build`), the six focused `migration_smoke` CTest
entries, the full 15-test public migration CTest suite including API checks and
...
six focused ASAN `migration_smoke` entries with `LSAN_OPTIONS=detect_leaks=0`.
The paired `mdbx_migration_bench_lazy` gate passed with forced/default ratios
of `1.181` batch, `1.156` crud, `1.263` iterate, `0.958` get, and `1.068`
delete.

A later dirty-write cache-invalidation cleanup removed
`dxb_iov_dirty_cache_invalidate_submit_io_t`,
`iov_make_dirty_cache_invalidate_submit_io()`,
`iov_dirty_cache_invalidate_submit_io_validate()`, `iov_data_write_io_equal()`,
and `iov_cache_invalidate_io_equal()` from `mdbx.c`. The dirty-page completion
callback now validates the queued data-write descriptor, builds and validates
the cache-invalidate descriptor, builds and validates its submit descriptor,
and submits the cache invalidation directly through storage while preserving
the same data-channel, storage ownership, page-span, and reusable-entry checks.
Verification passed `git diff --check`, a source scan proving the removed
dirty-write cache-invalidation helpers are absent from `mdbx.c`, the GNUmake
`mdbx_migration_smoke` build target, direct `mdbx_migration_smoke` default and
forced tiny-cache runs, the Ninja build (`cmake --build @cmake-ninja-build`),
the six focused `migration_smoke` CTest entries, the full 15-test public
migration CTest suite including API checks and tool roundtrips, the ASAN build
...
A later data-write submission cleanup removed
`dxb_write_cache_invalidate_submit_io_t`,
`dxb_storage_make_write_cache_invalidate_submit_io()`,
`dxb_storage_write_cache_invalidate_submit_io_validate()`, and the
write-cache-invalidation equality helpers from `mdbx.c`. The single-buffer and
vector write submit descriptors now carry only the checked write request plus
payload pointer/iovec metadata. `dxb_storage_submit_write_data()` and
`dxb_storage_submit_writev_data()` derive, validate, and submit the destination
page-cache invalidation after successful `pwrite()`/`pwritev()` completion,
leaving the cache invalidation at the same storage boundary as the actual
write. Verification passed `git diff --check`, a source scan proving the
removed write-invalidation submit helpers are absent from `mdbx.c`, the
GNUmake `mdbx_migration_smoke` build target, direct `mdbx_migration_smoke`
default and forced tiny-cache runs, the Ninja build (`cmake --build
@cmake-ninja-build`), the six focused `migration_smoke` CTest entries, the
full 15-test public migration CTest suite including API checks and tool
...
A later set-filesize-bytes cleanup removed
`dxb_filesize_set_bytes_submit_io_t`,
`dxb_storage_make_filesize_set_bytes_submit_io()`,
`dxb_storage_filesize_set_bytes_submit_io_validate()`, and the
set-bytes submit equality helper from `mdbx.c`. Setup and resize paths now
pass the plain validated `dxb_filesize_submit_io_t` request directly to
`dxb_storage_submit_set_filesize_bytes()`. The submitter captures old file
size, derives the filesize-state update, performs `ftruncate()`, invalidates
stale tail cache pages for shrink operations, and then updates storage
filesize state at the same storage boundary. Verification passed
`git diff --check`, a source scan proving the removed set-bytes submit helpers
are absent from `mdbx.c`, the GNUmake `mdbx_migration_smoke` build target,
direct `mdbx_migration_smoke` default and forced tiny-cache runs, the Ninja
build (`cmake --build @cmake-ninja-build`), the six focused
`migration_smoke` CTest entries, the full 15-test public migration CTest suite
