...
/** \brief Clone a read-only transaction to a new transaction handle.
 * \ingroup c_transactions
 *
 * Creates a new read-only transaction that views the same MVCC snapshot
 * as the source transaction. The cloned transaction has its own reader slot
 * and is completely independent from the source transaction - it can be
 * used, reset, renewed, or aborted independently.
 *
 ...
 *
 * \note Cloning is only supported for read-only transactions.
 ...
 * from any thread.
 *
 * \param [in] src   An active read-only transaction to clone from.
 *                   Must not be finished, parked, ousted, in error state,
 *                   or have child transactions.
 * \param [out] dest Address where the new \ref MDBX_txn handle will be stored.
 *                   On success, *dest points to a new read-only transaction
 *                   that sees the same snapshot as src.
 *
 * \returns A non-zero error value on failure and 0 on success,
 *          some possible errors are:
 * \retval MDBX_EINVAL           Invalid parameter (src is NULL, dest is NULL,
 *                               or src is not a read-only transaction).
 * \retval MDBX_BAD_TXN          Source transaction is finished, in error,
 *                               parked, or otherwise not in a valid state.
 * \retval MDBX_EBADSIGN         Transaction object has invalid signature.
 * \retval MDBX_THREAD_MISMATCH  Source transaction is not owned by the
